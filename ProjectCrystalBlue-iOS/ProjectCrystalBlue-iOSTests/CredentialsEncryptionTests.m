...
- (void)testEncryptAndDecryptString
{
    ...
    for (unsigned long i = 0; i < numberOfTests; ++i) {
        ...
        // Encrypt it
        NSData *encryptedData = [CredentialsEncryption encryptData:dataToEncrypt
                                                                 WithKey:key];

        // Check that encrypting has actually changed the data
        XCTAssertFalse([dataToEncrypt isEqualToData:encryptedData]);

        // Decrypt it
        NSData *decryptedData = [CredentialsEncryption decryptData:encryptedData
                                                                 WithKey:key];

        // We expect this to be the same as the data the we sent to be encrypted in the first place.
        XCTAssertTrue([dataToEncrypt isEqualToData:decryptedData]);
